...
class Test04TitleAPI:
    def test_01_title_not_auth(self, client):
        ...
        assert response.status_code != HTTPStatus.NOT_FOUND, (
            "Эндпоинт `/api/v1/titles/` не найден.Проверьте настройки в " "*urls.py*."
        )
    ...
    def test_03_titles_detail(self, client, admin_client):
        ...
        update_data = {"name": "Новое название", "category": categories[1]["slug"]}
    ...
    def test_05_titles_check_permission(
        ...
        check_permissions(
            moderator_client, url, data, "модератора", titles, HTTPStatus.FORBIDDEN
        )
