...
class Test02CategoryAPI:
    ...
    def test_02_category_with_admin_user(self, admin_client):
        ...
        assert response.status_code == HTTPStatus.OK, (
            f"Проверьте, что при GET-запросе к `{url}` возвращается статус " "200."
        )
        data = response.json()
    ...
    def test_03_category_fields_validation(self, data, massage, admin_client):
        url = "/api/v1/categories/"
        response = admin_client.post(url, data=data)
        assert response.status_code == HTTPStatus.BAD_REQUEST, massage[0].format(
            url=url
        )

    def test_04_category_delete_admin(self, admin_client):
        category_1, category_2 = create_categories(admin_client)
        response = admin_client.delete(f'/api/v1/categories/{category_1["slug"]}/')
        ...
        response = admin_client.get(f'/api/v1/categories/{category_2["slug"]}/')
        assert response.status_code == HTTPStatus.METHOD_NOT_ALLOWED, (
            "Проверьте, что GET-запросы к `/api/v1/categories/{slug}/` "
            "запрещены и возвращают ответ со статусом 405."
        )
        response = admin_client.patch(f'/api/v1/categories/{category_2["slug"]}/')
        assert response.status_code == HTTPStatus.METHOD_NOT_ALLOWED, (
    ...
    def test_05_category_check_permission_admin(
        ...
        check_permissions(
            moderator_client, url, data, "модератора", categories, HTTPStatus.FORBIDDEN
        )
